...
public:
	...
	void onUpdate(glm::mat4) override
	{
		...
		t->rotation *= glm::angleAxis(pitch, glm::vec3{1.0f, 0.0f, 0.0f});
	}
private:
	float yaw = 0.0f;
};
...
static void createMenu(SceneRoot& menu)
{
	...
	menu.getChild("text")->getComponent<Transform>()->position = {-1.0f, 0.90f, 0.0f};
	menu.createChild("text2")->createComponent<UI>()->m_text = "Press enter to play";
	menu.getChild("text2")->getComponent<Transform>()->position = {-1.0f, 0.70f, 0.0f};
	menu.createChild("text3")->createComponent<UI>()->m_text = "Press escape to quit";
	menu.getChild("text3")->getComponent<Transform>()->position = {-1.0f, 0.50f, 0.0f};

	menu.createChild("donut")->createComponent<Renderer>()->setMesh("meshes/donut.mesh");
	menu.getChild("donut")->getComponent<Transform>()->position = {0.0f, 0.0f, -5.0f};
	menu.getChild("donut")->getComponent<Transform>()->scale = {0.025f, 0.025f, 0.025f};
	menu.getChild("donut")->getComponent<Transform>()->rotation = glm::angleAxis(glm::half_pi<float>(), glm::vec3{1.0f, 0.0f, 0.0f});
...
*/
	...
	gunTransform->position = glm::vec3{0.2f, -0.1f, -0.15f};
	...
	floorTransform->scale = { 4.0f, 4.0f, 4.0f };
	...
	chunk->getComponent<Transform>()->position = {8.0f, 1.0f, 8.0f};
	...
	mainScene.getChild("detail_text")->getComponent<Transform>()->position = { 0.0f, -0.95f, 0.0f };
	mainScene.getChild("detail_text")->getComponent<Transform>()->scale = { 0.5f, 0.5f, 1.0f };
	...
	mainScene.getChild("cuboid")->getComponent<Transform>()->scale = {0.1f, 5.0f, 5.0f};
	mainScene.getChild("cuboid")->getComponent<Transform>()->position = {-65.0f, 5.0f, 0.0f};

	auto sunPivot = mainScene.createChild("sun_pivot");
	sunPivot->getComponent<Transform>()->position = { 0.0f, 10.0f, 0.0f };
	auto sun = sunPivot->createChild("sun");
	sun->getComponent<Transform>()->position = { 50.0f, 0.0f, 0.0f };
	...
	public:
		...
		{
			sunPivotTransform = parent->getParent()->getComponent<Transform>();
		}
		...
		void onUpdate(glm::mat4 t) override
		{
			...
			yaw += dt * speed;
			yaw = glm::mod(yaw, glm::two_pi<float>());
			sunPivotTransform->rotation = glm::angleAxis(yaw, glm::vec3{ 0.0f, 1.0f, 0.0f });
		}
	private:
